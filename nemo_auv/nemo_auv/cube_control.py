...
class CubeControl(Node):
    ...
    def timer_callback(self):
        ...
        # Get cube position information (you may need to replace these values with the actual cube positions)
        cube_x = 2.0
        cube_y = 3.0
        cube_z = 1.0

        # Calculate errors based on cube position
        depth_err = cube_z - self.get_depth()
        heading_err = math.atan2(cube_y, cube_x) - math.radians(self.get_heading())  # Calculate desired heading based on cube position
        x_error = cube_x - self.target_x  # Update x_error based on cube position

        # Calculate integral error (unchanged)
        self.depth_err_sum += depth_err
        self.heading_err_sum += heading_err

        # If error is sufficiently small, reset integral error (unchanged)
        if isclose(depth_err, 0.0, abs_tol=10):
            self.depth_err_sum = 0.0
        if isclose(heading_err, 0.0, abs_tol=3):
            self.heading_err_sum = 0.0

        # Calculate vertical throttle (unchanged)
        depth_throttle = 500 + self.Kp_depth * depth_err + self.Ki_depth * self.depth_err_sum

        # Calculate rotational throttle (unchanged)
        rot_throttle = self.Kp_a * heading_err + self.Ki_a * self.heading_err_sum

        # Run x controller at slower frequency because ... TODO (unchanged)
        x_throttle = self.Kp_x * x_error

        # From rotational throttle, edit x and y throttle to achieve rotation -> shouldn't have to do this (unchanged)
...
def main(args=None):
    ...
    # Create the node
    cube_control_node = CubeControl()

    # Spin the node so the callback function is called.
    rclpy.spin(cube_control_node)

    # Destroy the node explicitly
    # (optional - otherwise it will be done automatically
    # when the garbage collector destroys the node object)
    cube_control_node.destroy_node()
